...
class SSLMetaArch(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.is_3d = getattr(cfg.train, "is_3d", False)
    ...
    def get_teacher_output(
        ...
        *,
        ...
        if self.is_3d:
            n_crops, B, ch, D, H, W = images.shape
        else:
    ...
    def get_gram_teacher_output(self, images, *, masks, teacher_global, student_global, student_global_crops_size):
        ...
        if self.gram_ema_teacher:
            ...
            if self.is_3d:
                ...
                n_crops, B, rgb, H, W = images.shape
            images = images.flatten(0, 1)  # [n_crops * B, C, (D,) H, W]
            ...
            if teacher_patches.shape[1] != student_patches.shape[1]:
                ...
                if self.is_3d:
                    D_grid = D // self.cfg.student.patch_size
                    assert teacher_patches.shape[1] == N**2 * D_grid
                    D_student = D  # assume same depth for student
                    D_student_grid = D_student // self.cfg.student.patch_size
                    assert student_patches.shape[1] == N_student**2 * D_student_grid
                    patches_dhw = teacher_patches.transpose(-2, -1).unflatten(-1, (D_grid, N, N))
                    patches_dhw = torch.nn.functional.interpolate(
                        patches_dhw,
                        size=(D_student_grid, N_student, N_student),
    ...
    def get_student_output(self, *, global_crops, local_crops, upperbound, masks, mask_indices_list):
        if self.is_3d:
            n_global_crops, B, ch, D, H, W = global_crops.shape
            n_local_crops, B, ch, D, H, W = local_crops.shape
        else:
    ...
    def build_data_augmentation_dino(self, cfg):
        if getattr(cfg.train, "is_3d", False):
            return DataAugmentation3D_DINO(
