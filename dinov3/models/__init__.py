...
from . import vision_transformer_3d as vits3d
...
def build_model(args, only_teacher=False, img_size=224, device=None):
    if "vit3d" in args.arch:
        ...
        teacher = vits3d.__dict__[args.arch](**vit_kwargs)
        teacher = init_fp8(teacher, args)
        if only_teacher:
            return teacher, teacher.embed_dim
        student = vits3d.__dict__[args.arch](
            **vit_kwargs,
...
def build_model_from_cfg(cfg, only_teacher: bool = False):
    is_3d = getattr(cfg.train, "is_3d", False)
    if is_3d:
